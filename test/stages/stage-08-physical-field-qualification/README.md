# Stage 8 - Physical Field Qualification

**Status: not defined.** This directory records the CONOPS scope for the
stage.
The executable stage definition does not exist.

**Source:** CONOPS v1.01 section 78, Stage 8.

## Scope

From the CONOPS:

- antenna durability;
- tool-less replacement;
- glove use;
- night use and light discipline;
- connector keying;
- movement;
- snag resistance;
- thermal behavior;
- environmental resilience.

## Why this stage

The human-factors stage. CONOPS section 38 requires that incorrect RF
connections be prevented **mechanically or tactilely** rather than by colour
or label alone, which the prototype BOM addresses by deliberately mixing four
connector families so the bearers cannot be cross-connected.

CONOPS section 65 requires status illumination suppressible to off, and
forbids uncontrolled always-on indicators **including those integral to
selected commercial modules** — a BOM constraint as much as a software one.

SAD section 25.4.1 sets a six-feed antenna planning envelope, plus an optional
seventh for GNSS, and warns the enclosure must not be dimensioned around a
three-radio assumption.

## What it validates

- **Section 79 success criteria:** 15, 20, 31
- **Decisions:** `FML-ADR-045`
- **Trades expected to close or advance here:** `TBR-THERM-01`, `TBR-CARRIER-01`, `TBR-RF-02`

The criterion-to-stage mapping is CONOPS section 85 and is transcribed as
structured requirements in `docs/verification/requirements.md`, from which
`tools/gen-traceability.sh` generates the matrix.

## What a definition must contain

Per `test/stages/README.md`: a stable stage identifier, purpose, the
configuration under test (hardware block, compatibility set version, region
profile, mission profile), preconditions, a step-by-step procedure with
explicit
pass criteria, instrumentation, **pass criteria written before the stage is
run**, the evidence produced and where it is filed in `test/results/`, and
what
a failure means.

## Blocked on

Stage definitions depend on a selected hardware block (`TBR-HW-01`) and on a
populated requirement set. Neither exists. Defining pass criteria now would
mean
inventing thresholds for hardware nobody has chosen.

The exception is the promotion gate in `os/release/README.md`, which is a
build-acceptance gate rather than a qualification stage and exists today.
