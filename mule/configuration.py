"""Resolve bounded node configuration from explicit deployment inputs.

This is the real FML-ADR-083 runtime path. ``tools/gen-config.py`` is only a
builder-side command wrapper around this module.
...
def _catalog_references(
    ...
    """
    selected_catalog = catalog_path if catalog_path is not None else CATALOG_PATH
    selected_schema = (
        catalog_schema_path if catalog_schema_path is not None else CATALOG_SCHEMA_PATH
    )
    selected_quadlets = (
        quadlets_dir
        if quadlets_dir is not None
        else selected_catalog.parent.parent / "quadlets"
    )
    with selected_catalog.open(encoding="utf-8") as handle:
        catalog = yaml.safe_load(handle)
    with selected_schema.open(encoding="utf-8") as handle:
    ...
    for entry in entries:
        ...
        if enabled:
            ...
            for logical in logicals:
                if not (selected_quadlets / logical).is_file():
                    raise ConfigError(
                        f"enabled catalog service {name!r} deployment unit is absent: "
                        f"services/quadlets/{logical}"
                    )
                if (selected_quadlets / f"{logical}.disabled").is_file():
                    raise ConfigError(
            ...
            for logical in [unit, *[str(member) for member in bundle]]:
                if (selected_quadlets / logical).is_file():
                    raise ConfigError(
...
def _resolve_services(
    ...
    references = _catalog_references(catalog_path, catalog_schema_path, quadlets_dir)
...
def resolve(
    ...
    *,
    ...
    # for one.
    enabled = _resolve_services(
        list(mission.get("services", [])),
        catalog_path,
        catalog_schema_path,
        quadlets_dir,
    )
...
def generate(
    ...
    *,
    ...
    if out_dir is not None:
        ...
        pending = out_dir / ".parameters.json.pending"
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help="path to the approved service catalog",
    )
    parser.add_argument(
        "--catalog-schema",
        type=Path,
        default=CATALOG_SCHEMA_PATH,
        help="path to the service catalog schema",
    )
    parser.add_argument(
        "--quadlets",
        type=Path,
        default=CATALOG_PATH.parent.parent / "quadlets",
        help="directory containing loadable service-plane units",
    )
    parser.add_argument(
        "--check",
        action="store_true",
