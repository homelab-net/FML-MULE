...
1. The second half of the listed evidence is empirical: a different-node
   ...
   None has been performed. **Superseded 2026-09-05: the empirical half has
   since been performed** -- the different-node restore and all four workflow
   tests; see the README and the dated artifacts. This artifact remains the
   analysis half, and reason 2 still holds for it.
...
5. ~~**The four workflow tests** named in the trade: DataSync, mission package,
   certificate, map cache.~~ **Done.** Certificate and mission API 2026-08-31;
   mission-package and DataSync content 2026-08-31 and 2026-09-05
   (`2026-08-31-certificate-enrollment.md`,
   `2026-08-31-mission-api-and-the-header-that-authenticates.md`,
   `2026-08-31-datasync-and-package-workflows.md`,
   `2026-09-05-datasync-content-round-trip.md`). The map-cache is item 6.
6. ~~**Confirm the cache question empirically**: what a client observes after
   failover when the tile source is unreachable. Finding 4.~~ **Resolved by
   classification 2026-09-05.** OTS holds no tile state, so map/tile/cache is the
   EUD's own 26.3 client cache and the separate S1 map service, not TAK-server
