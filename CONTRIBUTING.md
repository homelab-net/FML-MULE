...
2. **Do not claim anything is tested.** No badges, no "working", no
   ...
   `UNVERIFIED` (no evidence), `SIMULATED` (exercised end to end on the software digital twin
...
`test/digital_twin/` is where that rule is cashed in: the real node logic composed
...
`test/digital_twin/README.md`. If you find yourself adding one to make a scenario
